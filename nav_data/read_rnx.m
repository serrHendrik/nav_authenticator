%**************************************************************************
% @par ESA Proprietary information. Unauthorized distribution, 
%      dissemination or disclosure not allowed. 
% @par Project: TGVF
%*************************************************************************/
function [H,OBSERVATIONS] = read_rnx(fileobs, PRN_selected)

% READ RINEX FILE version: 3.00 %
%
% OUTPUT %
% HEADER INFORMATION
% -------------------------------------------------------------------------------------------------
...
while 1
    ...
    if ~isempty(strfind(A,Obs_types_3))
        ...
        H.OBSERVATION_INF.OBS_TYPES {obs_types_counter,2} = sscanf(A(5:6),'%f');% # obs types
        nobst                = sscanf(A(5:6),'%f');
        obs_typ              = A(7:60);
   ...
   switch epoch_flag
       case {0, 1},
           ...
           for j = 1: num_lines
               ...
               if isempty(strfind('GRES',A(1))), error('More Satellite Observations expected in epoch %s',datestr(epoch')); end
               
               SatSys = A(1);
               satid  = strrep(A(1:3),' ','0');
               if any(strcmp(satid,PRN_selected))
                   ...
                   for k = 1: 16: N_typ * 16
                       ...
                       obs = sscanf(B(k:k+13),'%f');
                       lli = sscanf(B(k+14),'%f');
                       ssi = sscanf(B(k+15),'%f');
                       if isempty(obs) || (obs == 0.0), obs = NaN; end % missing obs
                       if isempty(lli) || (lli == 0), lli = NaN; end % missing obs
                       if isempty(ssi) || (ssi == 0), ssi = NaN; end % missing obs
